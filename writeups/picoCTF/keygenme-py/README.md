...
<a href='//mercury.picoctf.net/static/a6d9cac3bfa4935ceb50c145d3ff5586/keygenme-trial.py'>keygenme-trial.py</a>
...
Understanding the code, we can see that the functions `enter_license`, `check_key` and `decrypt_full_version` are the most interesting. If you run the program, you are prompted with different options and we could play around with them seeing what each one does.

```bash
┌──(arrow) 💀 [~/…/capture-the-flag/Writeups/picoCTF/keygenme-py]
...
Let's go with option c because that ends up decrypting something at the end if we enter the correct key.

The only problem is finding the correct key. As can be seen in the definition of `check_key` function, the check comparison takes place with some characters of the sha256 hash of the string "PRITCHARD" (bUsername_trial) and the two other static parts of the key. We can, therefore, find the key by hashing the string "PRITCHARD" and looking up characters at the relavant positions, and combining it with the static parts.
...
Running the above program, we get the key as:
...
Let's enter `picoCTF{1n_7h3_|<3y_of_54ef6292}` as the license key.
...
(c) Enter License Key
(d) Exit Arcane Calculator
What would you like to do, PRITCHARD (a/b/c/d)? c

Enter your license key: picoCTF{1n_7h3_|<3y_of_54ef6292}
...
The key works and we successfully managed to get access to the keygenme software without purchasing it!
